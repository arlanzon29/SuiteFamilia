 ...
 *
 * Lo único propio de Pendientes son el `base` y el `outDir`: `/SuiteFamilia/
 * pendientes/` en vez de `/SuiteFamilia/compra/`.
 */
...
/**
 * La raíz de la suite, dos niveles por encima de `apps/pendientes/`. De ahí
...
export default defineConfig(({ command, isPreview }) => {
  const outDir = raiz('dist/pendientes')
  return {
    base: command === 'build' || isPreview ? '/SuiteFamilia/pendientes/' : '/',
    envDir: raiz(''),
